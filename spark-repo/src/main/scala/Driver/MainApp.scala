    ...
    def main(args: Array[String]): Unit = {
        ...
        if (className isDefined) {
            try{
                ...
                case _: Exception => logger.error("class is not supported yet!")
            }
        } else {
    ...
    def parseCmd(args: Array[String]): Option[SparkParams] = {
        ...
        val cmdParser = {
            ...
            OParser.sequence(
             ...
             text(s"input format is required. example: Csv, Json etc. "),
            
             opt[String]('o', "output-format").required().valueName("<output-format>").
             action((x, c) => c.copy(outFormat = x)).
             text("output format is required, default to parquet."),
             ...
             text( """output save mode (append, overwrite, ignore)."""),
             ...
             opt[Map[String, String]]("output-options").optional().valueName("k1=v1,k2=v2...").action((x, c) =>
                c.copy(outOptions = x)).text("Spark read output options"))
            }
